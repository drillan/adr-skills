...
**既存状態（重要）:** リポジトリには既に未追跡の `pyproject.toml`（`name = "adr-plugin"`、`docs` 依存グループに Sphinx 系）、包括的な `.gitignore`、`uv.lock`、`docs/`（Sphinx プロジェクト）が存在する。`.gitignore` は `__pycache__` / `.pytest_cache` / `.ruff_cache` / `.mypy_cache` / `.venv` / `docs/_build/` を網羅済み（`uv.lock` は意図的に追跡対象）。

**Files:**
- Modify: `pyproject.toml`（既存を更新。新規作成しない）
- Create: `LICENSE`
- Create: `apm.yml`
- Create: `README.md`
- `.gitignore`: 既存のもので十分。変更しない。

- [ ] **Step 1: `pyproject.toml` を更新**

既存ファイルを次の内容に更新する。`name` を `adr-skills` に変更し、`description` を追加し、`dev` 依存グループと `[tool.*]` 設定を追加する。`docs` 依存グループと `requires-python` は保持する。
...
Run: `uv lock`
Expected: `uv.lock` が更新され、`dev` グループの pytest / ruff / mypy が解決される。
...
```bash
git add pyproject.toml uv.lock LICENSE apm.yml README.md
